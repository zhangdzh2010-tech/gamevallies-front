...
export default function CreativeStudio({
  ...
  useEffect(() => { setPublished(work?.status === 'published'); setShareUrl(work?.status === 'published' ? `${window.location.origin}${window.location.pathname}#${buildWorkExperiencePath(work.id)}` : ''); setPublishError(''); }, [work?.id, work?.status]);
  ...
  async function publish() {
    ...
    try {
      ...
      if (status === 'published') {
        setPublished(true);
        setShareUrl(`${window.location.origin}${window.location.pathname}#${buildWorkExperiencePath(work.id)}`);
