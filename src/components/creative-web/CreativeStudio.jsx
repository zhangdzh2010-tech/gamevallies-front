...
import { buildH5ShareUrl } from '../../utils/share';
...
export default function CreativeStudio({
  ...
  useEffect(() => { setPublished(work?.status === 'published'); setShareUrl(work?.status === 'published' ? buildH5ShareUrl(buildWorkExperiencePath(work.id)) : ''); setPublishError(''); }, [work?.id, work?.status]);
  ...
  async function publish() {
    ...
    try {
      ...
      if (status === 'published') {
        setPublished(true);
        setShareUrl(buildH5ShareUrl(buildWorkExperiencePath(work.id)));
