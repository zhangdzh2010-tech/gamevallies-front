...
export default function CreativeStudio({
  ...
  return <CreativeShell studio title={displayTitle} active="works" actions={<><span className="cw-save-state">{generating ? '正在创作' : loading ? '正在恢复' : session?.sessionId ? '创作描述已保存' : work?.id ? '作品已保存' : '描述尚未提交'}</span><button className="cw-button cw-primary" disabled={!completed || generating || loading} onClick={() => dialog.current?.showModal()}>{published ? '分享作品' : '发布作品'}<CreativeIcon name="arrow" /></button></>}>
    <main className="cw-studio"><section className="cw-conversation"><div className="cw-panelhead"><strong>创作对话</strong><span className="cw-badge">{mode === 'iterate' ? '继续完善' : mode === 'fork' ? '复刻创作' : session ? '确认方向' : '新的创意'}</span></div><div className="cw-conversation-body">
      ...
      {supplemental}
    </div><div className="cw-composer"><label htmlFor="creative-description">{session ? '确认或修改创作方向' : mode === 'iterate' ? '这一轮想改什么？' : mode === 'fork' ? '这次复刻想保留和改变什么？' : '描述你的创意'}</label><textarea id="creative-description" aria-label={inputAriaLabel} value={input} onChange={e => onInputChange?.(e.target.value)} disabled={!editable} maxLength={4000} placeholder={mode === 'fork' ? '描述想保留的核心，以及玩法、风格或体验上的变化。' : '想呈现什么现象？希望体验者可以调整什么、观察什么？'} /><div className="cw-composer-actions"><span>先确认方向，再开始创作</span><button className="cw-button cw-primary" data-testid="workspace-primary" disabled={!editable || primary?.disabled || !primary} onClick={primary?.onClick}>{primary?.label || '确认方向'}</button></div><div className="cw-secondary-actions">{secondary.map(action => <button key={action.key} data-testid={`secondary-${action.key}`} disabled={!editable || action.disabled} onClick={action.onClick}>{action.label}</button>)}</div></div></section>
