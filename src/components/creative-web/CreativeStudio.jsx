...
export default function CreativeStudio({
  ...
  return <CreativeShell studio title={displayTitle} active="works" actions={<><span className="cw-save-state">{generating ? '正在创作' : loading ? '正在恢复' : session?.sessionId ? '创作描述已保存' : work?.id ? '作品已保存' : '描述尚未提交'}</span><button className="cw-button cw-primary" disabled={!completed || generating || loading} onClick={() => dialog.current?.showModal()}>{published ? '分享作品' : '发布作品'}<CreativeIcon name="arrow" /></button></>}>
    <main className="cw-studio"><section className="cw-conversation"><div className="cw-panelhead"><strong>创作对话</strong><span className="cw-badge">{mode === 'iterate' ? '继续完善' : mode === 'fork' ? '复刻创作' : session ? '确认方向' : '新的创意'}</span></div><div className="cw-conversation-body">
      <div className="cw-assistant"><span className="cw-mark">G</span><strong>GameVallies</strong></div>
      <p className="cw-reply">{mode === 'iterate' ? '想让这件作品发生怎样的变化？描述这一轮要调整的内容，确认方向后再开始。' : mode === 'fork' ? '以原作为起点，告诉我哪些内容要保留，哪些体验要改变。确认方向后会生成一件属于你的新作品。' : '从一个现象、一条规律，或者一种想象开始。我们一起把它变成可以探索的作品。'}</p>
      ...
      {supplemental}
    </div><div className="cw-composer"><label className="cw-composer-label" htmlFor="creative-description">{session ? '确认或修改创作方向' : mode === 'iterate' ? '这一轮想改什么？' : mode === 'fork' ? '这次复刻想保留和改变什么？' : '描述你的创意'}</label><textarea className="cw-composer-input" id="creative-description" aria-label={inputAriaLabel} value={input} onChange={e => onInputChange?.(e.target.value)} disabled={!editable} maxLength={4000} placeholder={mode === 'fork' ? '描述想保留的核心，以及玩法、风格或体验上的变化。' : '想呈现什么现象？希望体验者可以调整什么、观察什么？'} /><div className="cw-composer-actions"><span>先确认方向，再开始创作</span><button className="cw-button cw-primary" data-testid="workspace-primary" disabled={!editable || primary?.disabled || !primary} onClick={primary?.onClick}>{primary?.label || '确认方向'}</button></div><div className="cw-secondary-actions">{secondary.map(action => <button className="cw-button cw-secondary-action" key={action.key} data-testid={`secondary-${action.key}`} disabled={!editable || action.disabled} onClick={action.onClick}>{action.label}</button>)}</div></div></section>
