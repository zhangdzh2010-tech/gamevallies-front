...
test('square cards pin the hot badge to the cover and show title plus author only', () => {
  ...
  expect(scss).not.toMatch(/\.cw-gallery-meta p \{[^}]*-webkit-line-clamp:2/);
  expect(scss).toMatch(/\.cw-gallery-author \{[^}]*display:flex/);
  expect(scss).toMatch(/\.cw-gallery-avatar \{[^}]*border-radius:50%/);
  expect(scss).toMatch(/\.cw-gallery-actions \{[^}]*opacity:0/);
  expect(scss).toMatch(/\.cw-square-card:hover \.cw-gallery-actions/);
