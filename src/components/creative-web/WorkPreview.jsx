...
export default function WorkPreview({ work, canPlay = true, onUnlock, busy = false, conversation = false, onEdit }) {
  ...
  if (conversation) {
    const content = html ? <PlayerLetterbox><WorkSandbox workId={work.id} className="work-player" key={`${work.id}:${work.version}:${reload}`} title={work.title || '交互作品'} html={html} sandbox="allow-scripts" referrerPolicy="no-referrer" allow="fullscreen" /></PlayerLetterbox> : <div className="preview-placeholder">{getGameCoverUrl(work) && <img src={getGameCoverUrl(work)} alt="作品封面" />}<div className="eyebrow">作品预览</div><h2>{work?.title || '作品正在准备中'}</h2><p>动手体验，发现变化，再继续完善。</p><button className="btn primary" onClick={start} disabled={loading || busy}>{loading ? '正在加载…' : canPlay ? '开始体验' : '查看解锁方式'}</button></div>;
