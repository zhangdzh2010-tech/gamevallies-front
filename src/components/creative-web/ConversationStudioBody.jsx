...
import PublicationFields, { isPublicationReady } from './PublicationFields';
import { BRAND_MARK_SRC } from '../common/BrandMark';
import { useStudioEmbed } from './StudioEmbedContext';

export default function ConversationStudioBody({ mode, displayTitle, onTitleChange, orientation, onOrientationChange, input, onInputChange, session, work, previewWork, generating, progress, loading, error, primary, secondary = [], onCancel, onNew, canPlay, onUnlock, quotaText, supplemental, inputAriaLabel, completed, publishing, published, publishError, shareUrl, publish, publicationDraft, onPublicationDraftChange, children, home = false, navigation, format: selectedFormat, onFormatChange, embedded = false, onOpenWork, active }) {
  const embed = useStudioEmbed();
  const hideChrome = embedded || Boolean(embed?.inPage);
  ...
  const awaitingConfirm = Boolean(session) && !generating && !loading && !shownError && !completed && !continueCreate && Boolean(primary);
  const confirmLabel = !primary?.label || primary.label === '开始生成' || primary.label === '确认创作描述'
    ? '确认并开始生成'
    : primary.label;
  ...
  const body = <>
    <main className="feed">{empty && supplemental && <div className="supplemental">{supplemental}</div>}
      {empty ? <section className="empty"><img className="assistant-icon" src={BRAND_MARK_SRC} alt="" /><h1>让一个想法，变得可以探索。</h1><p>描述一个现象、一条规律，或者一个天马行空的想法。</p><div className="empty-options">{[CREATIVE_DOMAINS[2],CREATIVE_DOMAINS[1],CREATIVE_DOMAINS[4]].map(item => <button key={item.id} onClick={() => {update(item.prompt);focus();}}>{item.title}</button>)}</div></section> : <section className="conversation">
        ...
        <div className="reply"><img className="assistant-icon" src={BRAND_MARK_SRC} alt="" /><div><div className="reply-title">智了空间<span className="status"><span className="status-dot"/><span>{status}</span></span></div>
        ...
        {awaitingConfirm && <div className="confirm-actions" data-testid="waiting-confirm-actions"><button type="button" className="btn" disabled={busy} onClick={edit}>修改描述</button><button type="button" className="btn primary" data-testid="waiting-confirm-primary" disabled={busy || primary.disabled} onClick={submit}>{confirmLabel}</button></div>}
    ...
    <dialog ref={publishDialog}><div className="dialog-head"><strong>{published?'分享作品':'让更多人，走进你的创意。'}</strong><button className="iconbtn" aria-label="关闭" disabled={publishing} onClick={()=>publishDialog.current?.close()}><ConversationIcon name="close"/></button></div><div className="dialog-content"><h3>{work?.title || displayTitle}</h3>{completed && !shareUrl && <PublicationFields draft={publicationDraft} onChange={onPublicationDraftChange} disabled={publishing} />}<p>公开发布后，作品会进入创意广场，其他用户可以体验，并在你允许复刻时继续创作。</p>{publishError && <div className="alert" role="status">{publishError}</div>}{shareUrl && <label>作品分享地址<input className="dialog-input" value={shareUrl} readOnly onFocus={e=>e.target.select()}/></label>}<button className="btn primary" disabled={publishing || Boolean(shareUrl) || !isPublicationReady(publicationDraft)} onClick={publish}>{publishing?'正在发布…':shareUrl?'已发布':'确认公开发布'}</button></div></dialog>{children}
  </>;
  if (hideChrome) {
    return <div className="conversation-embed">{home ? null : <div className="top-actions conversation-embed-actions">{studioActions}</div>}{body}</div>;
  }
  return <ConversationLayout title={displayTitle} workId={work?.id} onNew={onNew} navigation={navigation} onOpenWork={onOpenWork} active={active} actions={studioActions}>{body}</ConversationLayout>;
}
