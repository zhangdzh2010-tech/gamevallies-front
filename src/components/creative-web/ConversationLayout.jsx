...
export default function ConversationLayout({ children, title, actions, workId, onNew, navigation, active = 'home' }) {
  ...
  return <div className="conversation-app">
    ...
    <aside className="sidebar"><button className="brand" onClick={() => navigate('home')} aria-label="返回智了空间工作台"><span className="brand-mark">智</span>智了空间</button>
      ...
      <nav aria-label="主导航">{[['home','spark','创作空间'],['ideas','grid','发现灵感'],['works','folder','我的作品']].map(([id, icon, label]) => <button key={id} className={`nav${active===id?' active':''}`} onClick={() => navigate(id)}><ConversationIcon name={icon}/>{label}</button>)}</nav>
