...
用户已明确选择阿里云函数计算 FC，不使用 ECS。采用 OSS ZIP 代码包与 `custom.debian12`，不使用 ACR。Docker 仅可用于构建和解包测试。产品是面向桌面 Web 的交互创意平台，覆盖物理、生物、化学和艺术；不新增移动 H5 产品要求。内部 Taro H5 构建名称和 /games API 保留兼容。
...
1. Web/API 使用 FC Web 函数与 OSS ZIP 自定义运行时，保留服务端口和健康检查。前端可用 FC 托管构建产物；OSS/CDN 静态托管是可选优化，不擅自改为用户必需购买项。不要把 Compose 的多服务编排直接塞进一个函数。
2. 在 Debian 12/linux/amd64 构建可重定位代码包，包含根目录可执行 bootstrap、运行时与本地依赖；ZIP 上传同地域现有私有 OSS。检查地区包体限制（深圳 500 MB），AI Chromium 必须从解压后的包真实启动测试；超限拆层，不能擅自改回镜像。每个函数明确监听地址/端口、启动命令、健康检查和资源限制。
...
7. AI 代码包中的浏览器检查需验证 Chromium 依赖、内存、临时空间与启动耗时。按模型请求和数据库连接承载能力设置实例并发、最大实例数、超时、重试；不把平台最高限制当成默认配置。
...
- 现有私有 OSS Bucket、地域和应用隔离前缀；不要求 ACR 参数。
...
- GitHub 托管 Runner 构建测试、上传带提交 SHA 与摘要的不可变 OSS ZIP，再用经过验证且锁定版本的 Serverless Devs/FC CLI 或 SDK 发布；不依赖 ECS 自托管 Runner。Secrets 名称以新工作流的实际定义为准。
- ZIP 使用提交 SHA/不可变摘要，记录函数版本、配置版本和域名/触发器所指向的发布目标。保留上个已验证版本，回滚采用所选 FC 版本支持的版本/别名或配置切换；不承诺不存在的 API。配置、密钥、数据迁移分别管理，代码回滚不等于数据回滚。
